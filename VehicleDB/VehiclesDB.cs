...
namespace VehicleDB
{
    public class VehiclesDB
    {
        ...
        {
            return ReMatch(v.Type, Regex.Escape(sv)) || ReMatch(v.Brand,  Regex.Escape(sv));
        }
        ...
        public IEnumerable<VehicleBase> FindVType(string type, bool full = false)
        {
            return this.Find(v => ReMatch(v.GetType().Name, "^" + Regex.Escape(type) + (full ? "$" : "")));
        }
